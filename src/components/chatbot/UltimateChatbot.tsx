...
export function UltimateChatbot() {
  ...
  // follow the site-wide ES/EN switch (set by the studio LangProvider)
  useEffect(() => {
    const saved = window.localStorage.getItem("leyva-lang");
    if (saved === "en" || saved === "es") setLang(saved);
    const onSiteLang = (e: Event) => {
      const l = (e as CustomEvent<string>).detail;
      if (l === "en" || l === "es") setLang(l);
    };
    window.addEventListener("leyva:lang", onSiteLang);
    return () => window.removeEventListener("leyva:lang", onSiteLang);
  }, []);
